...
export function Pipeline() {
  ...
  return (
    <section id="akis" className="relative px-5 py-24 sm:px-8 sm:py-32">
      <div className="mx-auto max-w-6xl">
        ...
        <div ref={ref} className="relative mt-16 lg:mt-20">
          ...
          <div className="absolute top-2 bottom-2 left-[27px] w-px bg-line md:left-1/2 md:-translate-x-1/2">
            <motion.div
              style={{ height }}
          ...
          <ol className="space-y-10 md:space-y-16">
            {steps.map((s, i) => (
              <li
                key={s.n}
                className="relative flex items-start gap-5 md:grid md:grid-cols-[1fr_auto_1fr] md:items-center md:gap-0"
              >
                {/* düğüm — mobilde solda, masaüstünde ortada */}
                <div className="md:col-start-2 md:row-start-1 md:px-8">
                  <motion.span
                    initial={{ scale: 0, opacity: 0 }}
                    whileInView={{ scale: 1, opacity: 1 }}
                    viewport={{ once: true, margin: "-90px" }}
                    transition={{ duration: 0.5, delay: 0.1, ease: [0.34, 1.56, 0.64, 1] }}
                    className="relative flex size-14 items-center justify-center rounded-full border border-brand-500/35 bg-ink font-mono text-sm font-medium text-brand-300"
                  >
                    <span
                ...
                >
                  <div className="group glass rounded-2xl p-6 transition-all duration-300 hover:border-brand-400/40 hover:shadow-[0_0_40px_-12px] hover:shadow-brand-500/40">
                    <div className={`flex items-center gap-3 ${i % 2 ? "" : "md:flex-row-reverse"}`}>
                      <span className="flex size-10 shrink-0 items-center justify-center rounded-xl border border-brand-500/25 bg-brand-500/10 text-brand-300 transition-colors group-hover:border-brand-400/50 group-hover:text-accent">
