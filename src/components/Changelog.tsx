...
import { doldur, type Sozluk } from "@/lib/i18n";
...
function Notlar({ kayit, s }: { kayit: SurumKaydi; s: Sozluk }) {
  if (!kayit.notlar.length) {
    return <p className="mt-3 text-sm text-slate-500">{s.surumler.notYok}</p>;
  }
...
export function Changelog({ s, gecmis }: { s: Sozluk; gecmis: SurumKaydi[] }) {
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-8 flex flex-col items-stretch gap-3 sm:flex-row sm:items-center">
          {kalan > 0 && (
            ...
            >
              {doldur(s.surumler.dahaGoster, { sayi: kalan })}
            </button>
          )}
          <a
